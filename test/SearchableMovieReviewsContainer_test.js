...
describe('<SearchableMovieReviewsContainer />', () => {
  ...
  it('should have top-level element with class "searchable-movie-reviews"', () => {
   ...
   it('should fetch data from the New York Times API on form submission', () => {
    ...
    expect(fetchSpy.callCount > 0, "Fetch was not called").to.equal(true);	    expect(fetchSpy.callCount > 0, "Fetch was not called").to.equal(true);
    expect(fetchSpy.firstCall.lastArg, "Fetch should have the base URL 'https://api.nytimes.com/svc/movies/v2/reviews/all.json?'").to.include('https://api.nytimes.com/svc/movies/v2/reviews/all.json?')	    expect(fetchSpy.firstCall.lastArg, "Fetch should have the base URL 'https://api.nytimes.com/svc/movies/v2/reviews/search.json?'").to.include('https://api.nytimes.com/svc/movies/v2/reviews/search.json?')
  })
